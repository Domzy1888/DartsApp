...
###############################################################################
##### SECTION 3: CSS STYLING & CUSTOM THEME                               #####
###############################################################################
st.markdown(f"""
    <style>
    /* Background Image */
...
###############################################################################
##### SECTION 6: CORE DATA LOADING (PLAYER IMAGES & ADMIN)                #####
###############################################################################
if st.session_state['username'] != "":
    ...
    if menu == "Matches":
        if not admin_df.empty:
            ...
            if 'Cutoff' in admin_df.columns:
                try:
                    ...
                    if diff.total_seconds() > 0:
                        days, hours = divmod(int(diff.total_seconds()), 86400)
                        ...
                        st.markdown(f"""<div class="timer-container">
                            ...
                            <div class="timer-box"><span class="timer-val">{mins:02d}</span><span class="timer-label">Mins</span></div>
            ...
            if not user_sub.empty: is_locked = True
            ...
            # Only reveal next round if previous round is complete to reduce flicker
                ...
                if all(x != "Select Winner" for x in [sf1, sf2]):
                    ...
                    if final != "Select Winner" and not is_locked:
                        if st.button("LOCK PREDICTIONS"):
                            new_data = pd.DataFrame([{"Timestamp": datetime.now().strftime("%Y-%m-%d %H:%M"), "Username": st.session_state['username'], "Night": night_choice, "QF1": qf1, "QF2": qf2, "QF3": qf3, "QF4": qf4, "SF1": sf1, "SF2": sf2, "Final": final}])
                            conn.update(spreadsheet=URL, worksheet="User_Submissions", data=pd.concat([subs_df, new_data], ignore_index=True))
                            st.success("Predictions Saved!"); time.sleep(1); st.rerun()

    ###########################################################################
    ##### SECTION 8: LEADERBOARD PAGE LOGIC                               #####
    ###########################################################################
    ...
    ###########################################################################
    ##### SECTION 9: RIVAL WATCH LOGIC                                    #####
    ###########################################################################
    elif menu == "Rival Watch":
        st.title("👀 Rival Watch")
        if not admin_df.empty:
            watch_night = st.selectbox("Select Night", admin_df['Night'].unique())
            ...
            else: st.warning("Submit your own entries first to view rivals.")

    ###########################################################################
    ##### SECTION 10: HIGHLIGHTS LOGIC                                    #####
    ###########################################################################
    ...
    ###########################################################################
    ##### SECTION 11: ADMIN SCORING ENGINE                                #####
    ###########################################################################
    elif menu == "Admin":
        if st.session_state['username'].lower() == "admin":
            st.subheader("⚙️ Scoring Engine")
            st.info("System Ready for Night 1 Results.")
        else: st.error("Access Denied.")

# Welcome message if not logged in
else:
