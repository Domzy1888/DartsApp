...
# --- 2. COOKIE HELPER FUNCTION (The GitHub Fix) ---
# Instead of one global manager, we create a fresh one whenever we check/set cookies.
def get_manager():
    return stx.CookieManager(key=f"manager_{st.session_state.get('run_count', 0)}")
...
# --- 4. THE BOOTSTRAP LOGIN (Cookie Check) ---
# We do this at the very start of every script run
if st.session_state['username'] == "" and not st.session_state['logging_out']:
    # Fresh manager instance just for this check
    boot_manager = stx.CookieManager(key="boot_manager")
    # Give it a tiny bit more time to find the cookie on mobile
    time.sleep(0.5) 
...
# 5. Connection & Styling (Existing code)
...
# Setup Mute/Page from storage
# Note: We use the "boot_manager" from above to read these values too
saved_mute = stx.CookieManager(key="pref_read").get(cookie="pdc_mute")
...
if st.session_state['username'] == "":
    ...
    if st.sidebar.button("Go"):
        ...
        if not u_df.empty:
            ...
            if not match.empty:
                ...
                st.session_state['logging_out'] = False
                # Use a fresh, unique key for writing the login cookie
    ...
    if st.sidebar.button("Logout"):
        ...
        st.session_state['audio_played'] = False
        try:
            # Use a fresh instance just for deleting
            stx.CookieManager(key="delete_login").delete("pdc_user_login")
        except:
            pass
        st.rerun()

# --- SCORING & PAGES (Keeping your full logic here) ---
...
# Rival Watch and Admin pages follow existing logic...
